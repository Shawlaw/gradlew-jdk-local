...
4. **优先级说明**：在回复末尾简要说明 patch 后的 Wrapper / Client JVM 的 JDK 查找优先级：`local.properties` 中的 `java.home` > `.gradle/config.properties` 中的 `java.home` > 环境变量 `JAVA_HOME` > 系统 `PATH` 中的 `java`。
5. **回退行为说明**：在回复末尾简要说明：
...
6. **Daemon JVM 说明**：在回复末尾简要说明：Gradle Daemon / build JVM 仍遵循 Gradle 官方机制；如果没有配置 `org.gradle.java.home` 或 Daemon JVM criteria，Daemon / build JVM 默认也会使用本 patch 读到的 JDK；如果配置了这些官方选项，实际编译使用的 JDK 可能与 Wrapper / Client JVM 不同。
